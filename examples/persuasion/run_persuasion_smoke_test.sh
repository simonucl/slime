...
# SMOKE TEST VERSION - Uses small test dataset (20 train, 5 test tasks)
# For full training, use run_persuasion_qwen3_4B.sh instead
...
# SMOKE TEST: Use small test dataset instead of full dataset
export DATA_DIR=${DATA_DIR:-"${ROOT_DIR}/persuasion_data_test"}
...
# Checkpoint naming
CHECKPOINT_SUFFIX=${CHECKPOINT_SUFFIX:-"persuasion_smoke_test"}
...
ROLLOUT_ARGS=(
   ...
   --input-key index
   --rollout-shuffle
   # SMOKE TEST: Reduced rollout settings for quick validation
   --num-rollout 10  # Reduced from 300 for smoke test
   --rollout-batch-size 4  # Reduced from 8
   --n-samples-per-prompt 2  # Reduced from 8
   --rollout-max-response-len 4096
   --rollout-temperature 1
   --global-batch-size 8  # Reduced from 64
